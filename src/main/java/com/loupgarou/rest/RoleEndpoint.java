...
import com.loupgarou.model.Role;
...
public class RoleEndpoint {
	...
	public Response create(Role entity) {
		...
		return Response.created(
				UriBuilder.fromResource(RoleEndpoint.class)
						.path(String.valueOf(entity.getRoleId())).build())
				.build();
	}
	...
	public Response findById(@PathParam("id") Integer id) {
		TypedQuery<Role> findByIdQuery = em
				.createQuery(
						"SELECT DISTINCT r FROM Role r WHERE r.roleId = :entityId ORDER BY r.roleId",
						Role.class);
	...
	public List<Role> listAll(@QueryParam("start") Integer startPosition,
		...
		TypedQuery<Role> findAllQuery = em.createQuery(
				"SELECT DISTINCT r FROM Role r ORDER BY r.roleId", Role.class);
